...
const FETCH_INST_ADDR_URL  = (streetName,postcode,unitNo) =>`${API_BASE}/coverageCheck/${streetName}/${postcode}/${unitNo}`; 
...
//   })
	return axios.get(FETCH_INST_ADDR_URL(searchCriteria.streetBuildName,searchCriteria.postCode,searchCriteria.unitNo));
}
...
const _setCoverageCheckDataPopUp = (selectedAddress) => {
	return new Promise((resolve, reject) => {
	  resolve(selectedAddress);
	})
  }
  
  export const setCoverageCheckDataPopUp = (selectedAddress) => {
	return {
	  type: 'SET_COVERAGE_CHECK_DATA_POP_UP',
	  payload: _setCoverageCheckDataPopUp(selectedAddress)
	}
  }