...
import {
	...
	SET_COVERAGE_CHECK_DATA_POP_UP_PENDING, SET_COVERAGE_CHECK_DATA_POP_UP_FULFILLED, SET_COVERAGE_CHECK_DATA_POP_UP_REJECTED,
...
const initialMetaState = {
	...
	SET_COVERAGE_CHECK_DATA_POP_UP_STATUS: DEFAULT
}
...
function metaReducer(state = initialMetaState, action) {
	...
	switch (action.type) {
		...
		case SET_COVERAGE_CHECK_DATA_POP_UP_PENDING:
					return { ...state, SET_COVERAGE_CHECK_DATA_POP_UP_STATUS: PENDING };
		case SET_COVERAGE_CHECK_DATA_POP_UP_FULFILLED:
					return { ...state, SET_COVERAGE_CHECK_DATA_POP_UP_STATUS: SUCCESS };
		case SET_COVERAGE_CHECK_DATA_POP_UP_REJECTED:
					return { ...state, SET_COVERAGE_CHECK_DATA_POP_UP_STATUS: FAILED };
		default:
			return state;
	}
}
function dataReducer(state = initialDataState, action) {
	switch (action.type) {
		...
		case SET_COVERAGE_CHECK_DATA_FULFILLED:
			console.log('SET_COVERAGE_CHECK_DATA_FULFILLED');
			return {
				...
				orderId:null,addrId:null,btuId:null,createdDate:null,isOrderSubmitted:false,
				submittedOrderId:''
			}
		case SET_ORDERS_DATA_FULFILLED:
			return {
		...
		case CREATE_ORDER_FULFILLED:
				return {
					...state,submittedOrderId:action.payload.data,isOrderSubmitted:true
				}
		case SET_COVERAGE_CHECK_DATA_POP_UP_FULFILLED:
				return {
