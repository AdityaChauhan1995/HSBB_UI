...
class ReviewOrder extends Component {
	...
	constructor(props) {
		...
		this.state = {
			...
			isOrderSubmitted:props.isOrderSubmitted,
	...
	componentDidUpdate(prevProps) {
		console.log(prevProps.CREATE_ORDER_STATUS);
		if (prevProps.MODIFY_ORDER_STATUS != SUCCESS && this.props.MODIFY_ORDER_STATUS === SUCCESS) {
			this.setState({ loading:false,isOrderSubmitted:this.props.isOrderSubmitted , submittedOrderId:this.state.orderId});

		} else if (prevProps.MODIFY_ORDER_STATUS != FAILED && this.props.MODIFY_ORDER_STATUS === FAILED) {
			this.setState({loading:false,submissionErrorMessage:'Error occurred while submitting the order'})

		}
		if (prevProps.CREATE_ORDER_STATUS != SUCCESS && this.props.CREATE_ORDER_STATUS === SUCCESS) {
			this.setState({ loading:false,isOrderSubmitted:this.props.isOrderSubmitted ,submittedOrderId:this.props.submittedOrderId});

		} else if (prevProps.CREATE_ORDER_STATUS != FAILED && this.props.CREATE_ORDER_STATUS === FAILED) {
	...
	render() {
		...
		let { mode, btuId, orderData, isDisabled, custName,
			...
			installationAddress, custPackage, custEmail, custMobileNo,submittedOrderId,
			subscriberType, appointment, providerName,voip,tr69,iptv,
			isOrderSubmitted,submissionErrorMessage} = this.state;
									...
									<Grid style={{ background: 'white', paddingTop: 0 }}>
										<Grid.Row style={{ paddingBottom: 0 }}>
											<Grid.Column width={16}>
												...
												{ (isOrderSubmitted && submissionErrorMessage == '') && 
													<label style={{color:'Red'}}>ORDER SUBMITTED SUCCESSFULLY WITH ORDER ID - {submittedOrderId}</label> 
												}
												{ (submissionErrorMessage != '') && 
													<label style={{color:'Red'}}>{submissionErrorMessage}</label> 
												}
												<Divider />
											</Grid.Column>
										</Grid.Row>
...
const mapStateToProps = (state) => {
	return {
		...
		isOrderSubmitted:state.order.data.isOrderSubmitted,
		MODIFY_ORDER_STATUS: state.order.meta.MODIFY_ORDER_STATUS,
		CREATE_ORDER_STATUS: state.order.meta.CREATE_ORDER_STATUS,
		submittedOrderId:state.order.data.submittedOrderId,
	}
}
