...
class AppointmentBooking extends Component {
	...
	componentWillMount() {
		...
		let {shiftSelected,shiftDate,mode,isOrderSubmitted} = this.state;
		console.log('shiftSelected',shiftSelected,'shiftDate',shiftDate);
		...
		if(shiftDate === undefined || shiftDate === null){
			...
			var dt = new Date(shiftDate);
			//dt.setDate( dt.getDate() - 1 );
			const formattedDate = dt.toLocaleDateString('en-GB', {
				day: 'numeric', month: 'short', year: 'numeric'
			  }).replace(/ /g, '-');
			this.setState({shiftDate:dt,shiftFormattedDate:formattedDate});
		}
		if (mode == 'view' || isOrderSubmitted) {
